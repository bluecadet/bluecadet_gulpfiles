...
const config = {
  wordpress: {
    ...
    plugins: [          // Plugin Directory Names
      'test'
    ]
  },
  paths: {
...
//
// const get_directories  = dir_path => fs.readdirSync(dir_path).filter(f => fs.statSync(path.join(dir_path, f)).isDirectory());
const base_theme_path  = `web/wp-content/themes/`;
const base_module_path = 'web/wp-content/plugins/';

// const project_modules  = get_directories(base_module_path);
// const project_themes   = get_directories(base_theme_path);


config.wordpress.plugins.forEach((mod_dir) => {
  task_builder('plugin--' + mod_dir, base_module_path + mod_dir + '/');
});

config.wordpress.themes.forEach((theme_dir) => {
